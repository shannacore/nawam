...
BOOL CALLBACK LogCallback(HWND hDlg, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	EXT_DECL(log_ext, "nawam.log", __VA_GROUP__("*.log"), __VA_GROUP__("Nawam log"));
	switch (message) {
	case WM_INITDIALOG:
		...
		case IDCANCEL:
			...
			if ((!user_deleted_rufus_dir) && (log_size > 0) && ((log_buffer = (char*)malloc(log_size + 2)) != NULL)) {
				...
				if (log_size-- > 1) {
					...
					IGNORE_RETVAL(_mkdir(FILES_DIR));
					IGNORE_RETVAL(_chdir(FILES_DIR));
					FileIO(persistent_log ? FILE_IO_APPEND : FILE_IO_WRITE, "nawam.log", &log_buffer, &log_size);
				}
				safe_free(log_buffer);
			}
					...
					case ERROR_WRITE_FAULT:
						...
						if (PathFileExistsU(tmp)) {
							...
							if (i == ARRAYSIZE(ps_cmd)) {
								...
								uprintf("You may need to disable this feature, or add an exception, for Nawam to work...\n");
							}
						}
						break;
					}
...
static void PrintUsage(char* appname)
{
	...
	printf("  -g, --gui\n");
	printf("     Start in GUI mode (disable the 'nawam.com' commandline hogger)\n");
...
static HANDLE SetHogger(void)
{
	...
	if (hog_data != NULL) {
		...
		hogmutex = CreateMutexA(NULL, TRUE, "Global/Nawam_CmdLine");
...
#endif
{
	const char* rufus_loc = "nawam.loc";
	...
	static_sprintf(tmp_path, "%snawam.app", app_dir);
...
static const char _load_config_marker[16] __attribute__((used, aligned(16))) __attribute__((section(".rdata"))) = "_RUFUS_LOAD_CFG";
#if defined(_M_AMD64)
static const IMAGE_LOAD_CONFIG_DIRECTORY64 _load_config __attribute__((used, aligned(16))) __attribute__((section(".rdata"))) = {
	.Size = sizeof(IMAGE_LOAD_CONFIG_DIRECTORY64),
	.DependentLoadFlags = LOAD_LIBRARY_SEARCH_SYSTEM32
};
#else
static const IMAGE_LOAD_CONFIG_DIRECTORY32 _load_config __attribute__((used, aligned(16))) __attribute__((section(".rdata"))) = {
