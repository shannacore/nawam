 ...
 * License callback
 */
static void OpenDialogLink(HWND hDlg, LPARAM lParam)
{
	ENLINK* enl = (ENLINK*)lParam;
	wchar_t url[2048];
	TEXTRANGEW tr;
	LONG length = enl->chrg.cpMax - enl->chrg.cpMin;

	if (enl->nmhdr.code != EN_LINK || enl->msg != WM_LBUTTONUP ||
		length <= 0 || length >= ARRAYSIZE(url))
		return;
	tr.lpstrText = url;
	tr.chrg = enl->chrg;
...
INT_PTR CALLBACK LicenseCallback(HWND hDlg, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	HWND hLicense, hEdit;
	int i;
	char legal_notice[8192];
	const int edit_id[] = { IDC_ABOUT_BLURB, IDC_ABOUT_COPYRIGHTS };
	const char* edit_text[] = { legal_notice, additional_copyrights };
	...
	case WM_INITDIALOG:
		...
		SetWindowTextU(hDlg, "Nawam - License & Open Source");
		SetTitleBarIcon(hDlg);
		CenterDialog(hDlg, NULL);
		ResizeButtonHeight(hDlg, IDCANCEL);
		// Suppress any inherited RTL flags for the verbatim English GPL.
		...
		SendMessage(hLicense, EM_SETLIMITTEXT, (WPARAM)strlen(gplv3), 0);
		SetDlgItemTextA(hDlg, IDC_LICENSE_TEXT, gplv3);
		static_sprintf(legal_notice, legal_notice_format, lmprintf(MSG_176 | MSG_RTF));
		for (i = 0; i < ARRAYSIZE(edit_id); i++) {
			hEdit = GetDlgItem(hDlg, edit_id[i]);
			SendMessage(hEdit, EM_AUTOURLDETECT, 1, 0);
			SendMessageA(hEdit, EM_SETTEXTEX, (WPARAM)&friggin_microsoft_unicode_amateurs, (LPARAM)edit_text[i]);
			SendMessage(hEdit, EM_SETEVENTMASK, 0, ENM_LINK);
			SendMessage(hEdit, EM_SETBKGNDCOLOR, 0, (LPARAM)GetSysColor(COLOR_BTNFACE));
...
INT_PTR CALLBACK AboutCallback(HWND hDlg, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_INITDIALOG:
		...
		// Keep the expanded legal entry point explicit in every locale.
		SetDlgItemTextU(hDlg, IDC_ABOUT_LICENSE, "License && Open Source");
		SetTitleBarIcon(hDlg);
		CenterDialog(hDlg, NULL);
		...
		hEdit = GetDlgItem(hDlg, IDC_ABOUT_BLURB);
		SendMessage(hEdit, EM_AUTOURLDETECT, 1, 0);
		// RichEdit20W requires EM_SETTEXTEX for UTF-8 RTF.
		SendMessageA(hEdit, EM_SETTEXTEX, (WPARAM)&friggin_microsoft_unicode_amateurs, (LPARAM)about_blurb);
		SendMessage(hEdit, EM_SETEVENTMASK, 0, ENM_LINK);
		SendMessage(hEdit, EM_SETBKGNDCOLOR, 0, (LPARAM)GetSysColor(COLOR_BTNFACE));
		SendMessage(hEdit, EM_SETSEL, 0, 0);
		SetDarkModeForChild(hDlg);
...
{
#if !NAWAM_SELF_UPDATE_ENABLED
	uprintf("Nawam: automatic application updates are disabled.");
	return FALSE;
#else
	BOOL enable_updates;
...
INT_PTR CALLBACK NewVersionCallback(HWND hDlg, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_COMMAND:
		...
		case IDCANCEL:
			...
			return (INT_PTR)TRUE;
		case IDC_WEBSITE:
			ShellExecuteA(hDlg, "open", NAWAM_WEBSITE, NULL, NULL, SW_SHOWNORMAL);
			break;
...
{
#if !NAWAM_SELF_UPDATE_ENABLED
	uprintf("Nawam: executable update download is disabled.");
	return;
#else
