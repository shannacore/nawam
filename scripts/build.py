...
from release_support import make_provenance, require, source_snapshot, validate_pe

parser = argparse.ArgumentParser()
parser.add_argument("--clean", action="store_true", help="Compatibility flag; every build now removes generated objects/libraries")
...
with log_path.open("w", encoding="utf-8") as log:
    ...
    inputs = source_snapshot(ROOT, include_generated=False)
    # Dependency tracking is disabled upstream, including static libraries.
    # Always invalidate generated objects so provenance cannot bless stale code.
    ...
    for directory in (ROOT / "src", ROOT / ".mingw"):
        for file in directory.rglob("*"):
            if (file.is_file() and file.suffix in (".o", ".a", ".lib")
                ...
                file.unlink()
    (ROOT / "src/rufus.exe").unlink(missing_ok=True)
    (ROOT / "res/loc/embedded.loc").unlink(missing_ok=True)
    ...
    require(inputs == source_snapshot(ROOT, include_generated=False),
            'Build source changed during compilation; rebuild')
    provenance = make_provenance(ROOT, data, sources)
    provenance_text = json.dumps(provenance, indent=2) + '\n'
    staged_provenance = ROOT / 'dist/Nawam.build.provenance.json'
    staged_provenance.write_text(provenance_text, encoding='utf-8')
    os.replace(staged, output)
    require(output.read_bytes() == data, 'Canonical executable SHA256 verification failed')
    os.replace(staged_provenance, ROOT / 'docs/build-provenance.json')
